...
async function migrate() {
  ...
  -- backfill for nulls with row_number() based on created_at,id
  await pool.query(`
    WITH ranked AS (
      SELECT id, ROW_NUMBER() OVER (ORDER BY created_at ASC, id ASC) AS rn
      FROM artworks
    )
    UPDATE artworks a SET sort_order = r.rn
    FROM ranked r
...
app.post('/api/artworks', authMiddleware, upload.single('image'), async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO artworks (title, author, image) VALUES ($1,$2,$3) RETURNING id, title, author, created_at`,
      [title, author, req.file.buffer]
    );
    res.json(rows[0]);
  } catch (e) {
    console.error(e);
...
app.put('/api/artworks/:id', authMiddleware, upload.single('image'), async (req, res) => {
  try {
    ...
    const { title, author } = req.body;
    if (!title || !author) return res.status(400).json({ error: 'title/author required' });
    // If image uploaded, replace it; else keep existing
    if (req.file) {
...
app.put('/api/artworks/order', authMiddleware, async (req, res) => {
  try {
    ...
    // Use transaction to update sort_order
